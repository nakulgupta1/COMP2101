...
# Data Collection/Generation Section
myHostname=$(hostname)
myLANInterface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
myLANAddress=$(ip a s $myLANInterface |awk '/inet /{gsub(/\/.*/,"");print $2}')
myLANHostname=$(getent hosts $myLANAddress |awk '{print $2}')
cmdGetExternalIP="curl -s icanhazip.com"
myExternalIP=$($cmdGetExternalIP)
myExternalName=$(getent hosts $myExternalIP |awk '{print $2}')
...
# Adding the IP and name of the default router to my /etc/hosts file
myDefaultRouterIP=$(/sbin/ip route | awk '/default/ { print $3 }')
myDefaultRouterName=$(getent hosts $myDefaultRouterIP |awk '{print $2}')
echo -e "127.0.0.1\tlocalhost" > ./temp_hosts
echo -e "127.0.1.1\t$myHostname" >> ./temp_hosts
echo -e "$myDefaultRouterIP\t$myDefaultRouterName" >> ./temp_hosts
cat /etc/hosts | tail -n +3 >> ./temp_hosts
